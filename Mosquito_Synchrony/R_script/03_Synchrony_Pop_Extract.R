...
at1 %>%  filter(Year == "2018" ) %>% filter(Total_Count >10) %>% 
  ggplot( aes(x=DOY, y=Count, color= SciName)) + geom_point(size=2, alpha=.5)+
   facet_wrap(~Year, scales = 'free_y') 



at1 %>%  filter(Year == "2014" ) %>% filter(Total_Count >10) %>% 
  ggplot( aes(x=DOY, y=log10(Count+1), color= SciName)) + 
    geom_point(size=2, alpha=.5)+
    facet_wrap(~SciName, scales = 'free_y')  + 
    theme(legend.position = "none")

test.df <- at1 %>%  filter(Year == "2019" ) %>% filter(Total_Count >10) 
...
long.df %>%  
  ggplot( aes(x=DOY, y=log10(Count+1),color=SciName))+
  geom_line(size=1,alpha=.5) +facet_wrap(~Site,scales="free_y")+
  theme(legend.position = "none")
...
full.df %>% filter(Year=="2018" & DOY <300) %>% 
 filter( Site !="ORNL" & Site != "SERC" & Site != "TALL" ) %>% 
ggplot( aes(x=DOY,y=log10(Count+1), color=SciName))+geom_line(size=2,alpha=.5)+
  facet_wrap(~Site,scales="free_y")+theme(legend.position = "none")+
 scale_color_ordinal(name="Species name") +
  xlab("DOY") + ylab("Estimated log10 mosquito population") + 
  theme_classic()+
...
for( s in 1:length(unique(at1$Site))){
  ...
  site.df <- at1 %>% filter( Site == focSite)
    ...
    for(y in 1:length(unique(site.df$Year))){
     ...
     spp.combo <- as.data.frame(expand.grid(unique(pred.df$SciName),
  ...
  plot1 <-  ggplot() +
    ...
    geom_ribbon( data = trial.df, aes( x = DOY, ymax = Sp1, ymin=0),
                 alpha = .5, fill = "light blue") + 
    geom_line( data = trial.df, aes( x = DOY, y = Sp2 ) )+
    geom_ribbon( data = trial.df, aes( x = DOY, ymax = Sp2, ymin=0),
                 alpha = .5, fill= "red") +
    xlab( "Day of year" ) + theme_classic() +
...
overlap.df %>% filter(Overlap < 1 ) %>% 
ggplot( aes(x=Year,y=Overlap,fill=Site)) + geom_boxplot() +
   xlab("Year") + ylab("Population overlap") + 
  theme_classic()+
  theme( legend.position = "none",
         axis.line.x = element_line(color="black") ,
         axis.ticks.y = element_line(color="black"),
...
overlap.df %>% filter(Overlap < 1 ) %>% 
ggplot( aes(x=Overlap, fill=Year)) + geom_density(alpha=.75)+
  facet_wrap(~Site)+ xlab("Population overlap") + ylab("Density") + 
  theme_classic()+
  theme( legend.position = "none",
         axis.line.x = element_line(color="black") ,
         axis.ticks.y = element_line(color="black"),
...
overlap.df %>% filter(Overlap < 1 ) %>% 
  ggplot(aes(x=reorder(Sp1,-Overlap),y=Overlap)) + geom_boxplot(outlier.shape=NA) + geom_jitter(width=.1,alpha=.25)+
 xlab("Species identity") + ylab("Overlap") + 
...
overlap.df %>% filter(Overlap < 1 ) %>% 
  ggplot(aes(y=reorder(Comb,-Overlap),x=Overlap,color=Year)) + geom_point(alpha=.25)+
  xlab("Species identity") + ylab("Overlap") + 
  theme_classic()+ facet_wrap(~Site, scales="free_y") +
  theme( legend.position = "none",
         axis.line.x = element_line(color="black") ,
         axis.ticks.y = element_line(color="black"),
         axis.ticks.x = element_line(color="black"),
