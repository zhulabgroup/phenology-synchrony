...
p_map <- ggplot() +
  ...
  coord_equal()
p_map
pdf(paste0(path, "output/map.pdf"), width = 8, height = 8)
print(p_map)
dev.off()
...
for (i in 1:length(sp_list)) {
  ...
  corr_df <- as.data.frame(res) %>%
    ...
    rowwise() %>%
    mutate(
      distance = distm(c(start_lon, start_lat), c(end_lon, end_lat), fun = distHaversine) %>% as.numeric() %>% `/`(100000)
    ) %>%
    ungroup() %>%
    filter(distance > 0) %>%
    drop_na(correlation)
  ...
  model <- nlsLM(correlation ~ SSasymp(distance, yf, y0, log_alpha),
    data = corr_df,
