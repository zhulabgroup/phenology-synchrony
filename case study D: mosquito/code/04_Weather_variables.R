...
weather.df <- weather.df %>%  group_by(Site, Year, Lat, Long, DOY, Julian) %>% 
  summarise(
    PPT = mean(PPT),
    TMAX = mean(TMAX),
    TMean7 = mean(Tmean7),
...
photo <- ggplot(weather.df ,aes(x=DOY, y=Photoperiod,color=Site,group=(SiteYear) )) + 
  geom_line(size=2,alpha=.5) + facet_wrap(~Photo)+
  xlab("DOY") + ylab("Day length (hrs)") + 
  theme_classic()+
  theme(
...
precip <- ggplot(weather.df ,aes(x=DOY, y=PPT14,color=Site,group=SiteYear ))  + 
  geom_line(size=2,alpha=.5) + facet_wrap(~Precip)+
  xlab("DOY") + ylab("Total 14 day precip (mm)") + 
  theme_classic()+
  theme(
...
temp <- ggplot(weather.df ,aes(x=DOY, y=TMean7,color=Site,group=SiteYear ))   + 
  geom_line(size=2,alpha=.5) + facet_wrap(~Temp)+
  xlab("DOY") + ylab("7 day mean temperature (C)") + 
  theme_classic()+
  theme(
    legend.position = "none",
    legend.key.size = unit(1,"cm"),
...
ggplot(plot.df, aes( x=DOY, y=Count))+ geom_line()+
...
for( s in 1:length(unique(at1$Site))){
  ...
  for(y in 1:length(unique(site.df$Year))){
    ...
    weather.combo <- as.data.frame(expand.grid(unique(pred.df$SciName),
                                           "Temp"))
...
overlap.df <- overlap.df %>% filter(Site == "HARV" |
                                      ...
                                      Site == "WREF" |
                                      Site == "YELL")
ggplot(overlap.df, aes(x=Year,y=Corr,fill=Site)) + theme_classic()+
  geom_hline(yintercept = 0, alpha=.5)+geom_boxplot() +
  xlab("Abiotic variables") + ylab("Spearman correlation")+
...
overlap.df %>% filter(Site == "UNDE" ) %>%
  filter( SciName == "Aedes communis" |
           SciName == "Aedes punctor"|
           SciName== "Coquillettidia perturbans") %>% 
ggplot( aes(x=Abio,y=Corr,color=Year)) + theme_classic()+
  geom_hline(yintercept = 0, alpha=.5)+ geom_boxplot(color="black")+geom_point(size=3) +
...
overlap.df %>% 
  ggplot( aes(x=Corr, fill=Abio)) + geom_density(alpha=.75)+
  facet_wrap(~Site)+ xlab("Abiotic overlap") + ylab("Density") + 
  theme_classic()+
...
overlap.df %>% filter(Overlap < 1 ) %>% 
  ggplot(aes(x=reorder(Sp1,-Overlap),y=Overlap)) + geom_boxplot() + geom_jitter(width=.1)+
