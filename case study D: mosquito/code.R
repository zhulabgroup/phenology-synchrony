...
p_ts <- ggplot() +
  geom_line(
    data = count.df %>% filter(Site == "HARV", SciName == "Aedes trivittatus"),
    aes(x = DOY, y = cumCount, col = Year)
  ) +
  geom_rect(
    data = range.df %>% filter(Site == "HARV", SciName == "Aedes trivittatus"),
...
if (!file.exists(paste0(path, "data/daymet.rds"))) {
  ...
  for (r in 1:nrow(site_df)) {
    daymet_df_list[[r]] <- download_daymet(
      site = site_df$Site[r],
      lat = site_df$lat[r],
      lon = site_df$lon[r],
      start = 2014,
...
p_corr <- ggplot() +
  ...
  geom_smooth(
    data = range.df.proc %>% left_join(reg_df, by = "SciName") %>% filter(p < 0.05),
